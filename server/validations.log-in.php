...
if(!empty($_POST["iniciar"])){
    ...
    if(empty($_POST["username"]) || empty($_POST["password"])){
        ...
        $sql=$conection->query("SELECT * FROM usuarios WHERE nombre='$usuario' AND contrasena='$contra'");
        #si son correctos se redirecciona a la pagina de QRHerramientas.php
        if($datos=$sql->fetch_object()){
            echo'<script type="text/javascript">window.location.href="test.window.php";</script>';
