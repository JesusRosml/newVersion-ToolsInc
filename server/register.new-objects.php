...
if(!empty($_POST["enviar"])){
    ...
    $query = $conection->query("SELECT id FROM herramientas WHERE id = '$idherramienta'");
    if($query->num_rows > 0){
        echo "<script>alert('El id de esa herramienta ya existe, intente con otro id');</script>";
        echo "<script>history.go(-1);</script>";
    }else{
        $sql=$conection->query("INSERT INTO herramientas (id, nombre, color, tipo, fecha, descripcion) VALUES ('$idherramienta', '$nombreherramienta', '$colorherramienta', '$tipodeherramienta', '$fechadeherramienta', '$descripcionherramienta')"); 
    }
...
if(!empty($_POST["enviar1"])){
    ...
    $query = $conection->query("SELECT id FROM trabajadores WHERE id = '$idtrabajador'");
    if($query->num_rows > 0){
        echo "<script>alert('El id de ese trabajador ya existe, intente con otro id');</script>";
        echo "<script>history.go(-1);</script>";
    }else{
        $sql=$conection->query("INSERT INTO trabajadores (id, nombre, telefono, correo, fecha_registro, puesto) VALUES ('$idtrabajador', '$nombretrabajador', '$telefonotrabajador', '$correotrabajador', '$fechatrabajador', '$puestotrabajador')");
    }
...
if(!empty($_POST["actualizar"])){
    ...
    $fechadeherramienta = $_POST["date_tool"];
    $descripcionherramienta = $_POST["description_tool"];
    $sql=$conection->query("UPDATE herramientas SET nombre = '$nombreherramienta' , color = '$colorherramienta' , tipo = '$tipodeherramienta', fecha = '$fechadeherramienta', descripcion='$descripcionherramienta' WHERE id = '$idherramienta'");
}
if(!empty($_POST["actualizar2"])){
    ...
    $fechatrabajador = $_POST["date_worker"];
    $puestotrabajador = $_POST["work_worker"];
    $sql=$conection->query("UPDATE trabajadores SET nombre = '$nombretrabajador' , telefono = '$telefonotrabajador' , correo = '$correotrabajador', fecha_registro = '$fechatrabajador', puesto ='$puestotrabajador' WHERE id = '$idtrabajador'");
}
...
if(!empty($_POST["button_prestamo"])){
    ...
    for ($i = 1; $i <= $contador; $i++) {
        ...
        $sql=$conection->query("INSERT INTO prestamo (idtrabajador, idherramienta, fechaprestamo, fechafinalizacion, cantidad) VALUES ('$idtrabajador','$idherramienta','$fecha','$fechafinal','$cantidad')");
    }
...
if(!empty($_POST["prestamo_delete"])){
    ...
    $sql=$conection->query("DELETE FROM prestamo WHERE idprestamo = '$idprestamo'");
}
?>